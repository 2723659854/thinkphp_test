...
class Auth {
    ...
    protected $config = array(
        ...
        'auth_group'        => 'permission_auth_group',        // 用户组数据表名
        'auth_group_access' => 'permission_auth_group_access', // 用户-用户组关系表
        'auth_rule'         => 'permission_auth_rule',         // 权限规则表
        'auth_user'         => 'permission_admin',             // 用户信息表
    );
     ...
     */
    public function check($name, $uid,$num = 1, $type = 1, $mode = 'url', $relation = 'or') {
        if (!$this->config['auth_on']) {
            return true;
        }
        $authList = $this->getAuthList($uid, $type); //获取用户需要验证的所有有效规则列表    没有的权限
        if (is_string($name)) {
...
//            }
    ...
    public function getGroups($uid) {
        ...
        $user_groups = Db::table($this->config['auth_group_access'])
            ->alias('ga')
            ->field('ga.uid,ga.group_id,g.title,g.rules')
            ->join(config("database.database").'.permission_auth_group g','ga.group_id=g.id')
            ->where(['ga.uid' => $uid, 'g.status' => 1])
            ->select();
...
//            ->select();
    ...
    protected function getAuthList($uid, $type) {
        ...
        //读取用户组所有权限规则
        $rules = Db::table($this->config['auth_rule'])->where($map)->field('condition,name')->select();
        //循环规则，判断结果。
        $authList = [];   //
    ...
    protected function getUserInfo($uid) {
        ...
        if (!isset($userinfo[$uid])) {
            $userinfo[$uid] = Db::table($this->config['auth_user'])->where(['uid' => $uid])->find();
        }
        return $userinfo[$uid];
    }
}